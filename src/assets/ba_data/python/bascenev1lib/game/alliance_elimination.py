...
class AllianceEliminationGame(bs.TeamGameActivity[Player, Team]):
    ...
    def __init__(self, settings: dict):
        ...
        self._players_per_team_in_arena = int(settings["Players Per Team In Arena"])
    ...
    def handlemessage(self, msg: Any) -> Any:
        if isinstance(msg, bs.PlayerDiedMessage):
            ...
            player.lives -= 1
            if player.lives < 0:
                logging.exception("Got lives < 0 in Elim; this shouldn't happen.")
                player.lives = 0
            ...
            if player.lives == 0:
                ...
                # Ensure player is removed from spawn order when dead
                if player in player.team.spawn_order:
                    player.team.spawn_order.remove(player)

            # In solo, put ourself at the back of the spawn order.
            else:
                # Put ourself at the back of the spawn order (safely).
                spawn = player.team.spawn_order
                # remove if present, ignore if not
                if player in spawn:
                    spawn.remove(player)
                # ensure we don't create duplicates
                if player not in spawn:
                    spawn.append(player)
