...
class RaceRegion(bs.Actor):
    ...
    def __init__(self, pt: Sequence[float], index: int):
        ...
        # Ensure we're running inside a race-style activity that defines
        # a race_region_material attribute.
        assert hasattr(
            activity, "race_region_material"
        ), "RaceRegion must be created within a race activity"
        self.pos = pt
        self.index = index
...
class SquidRaceGame(bs.TeamGameActivity[Player, Team]):
    ...
    def __init__(self, settings: dict):
        ...
        # Squid-red-light tuning: pre-warning and grace after red.
        self._pre_red_warn = 0.4  # seconds before red to show warning
        self._red_grace = 0.4  # seconds after red before enforcing movement
        self._orig_tint = None  # will capture original globals tint on first warn
                ...
                if last_region >= len(self._regions) - 2 and this_region == 0:
                    ...
                    if player.lap == self._laps:
                        ...
                        if isinstance(self.session, bs.DualTeamSession):
                            ...
                            if self._team_finish_pts > 0:
                                self.stats.player_scored(
                                    player, self._team_finish_pts, screenmessage=False
                                )
                        ...
                        if team.lap == self._laps:
                            ...
                            # Match race.py behavior: end promptly when a team finishes.
        ...
        if self._mine_spawning != 0:
            ...
            if self._race_mines:
                self._race_mine_timer = bs.Timer(
                    0.001 * self._mine_spawning, self._update_race_mine, repeat=True
                )
    ...
    def _squidgame_countdown(self) -> None:
        self._countdown_timer = 60 * self._laps  # 60
        bs.newnode(
            "image",
            attrs={
                "opacity": 0.7,
    ...
    def _update_sgcountdown(self) -> None:
        ...
        if self._countdown_timer == 35:
            self._sq_mode = "Hard"
            bs.getsound("alarm").play()
        if self._countdown_timer == 50:
    ...
    def _start_squid_game(self) -> None:
        ...
        # Restore original world tint on green if we changed it before.
        try:
            activity = bs.get_foreground_host_activity()
            if getattr(self, "_orig_tint", None) is not None:
                activity.globalsnode.tint = self._orig_tint
        except Exception:
            pass
        self._do_delete = False
        self._ticks = True
        # Show a pre-warning (orange) shortly before red.
        if self._pre_red_warn > 0.0:
            warn_delay = max(0.0, random_number - self._pre_red_warn)
            bs.timer(warn_delay, self._pre_red_warning)
        bs.timer(random_number, self._stop_squid_game)

    def _pre_red_warning(self) -> None:
        # Briefly tint squid lights to orange as a heads-up before red.
        try:
            self._squid_lights[1].color = (1.0, 0.6, 0.0)
            # Optional subtle beep to warn.
    ...
    def _stop_squid_game(self) -> None:
        ...
        self._squid_lights[1].color = (0.0, 0.3, 0)
        # Apply red world tint when light actually turns red.
        try:
            activity = bs.get_foreground_host_activity()
            if getattr(self, "_orig_tint", None) is None:
                try:
                    self._orig_tint = activity.globalsnode.tint
                except Exception:
                    self._orig_tint = (1.1, 1.2, 1.3)
            activity.globalsnode.tint = (0.9, 0.4, 0.5)
        except Exception:
            pass
        # Give players a short grace window before enforcing no-move checks.
        bs.timer(self._red_grace, self._check_delete)
    ...
    def _check_delete(self) -> None:
        ...
        bs.timer(3.0 if self._sq_mode == "Hard" else 4.0, self._start_squid_game)
    ...
    def spawn_player(self, player: Player) -> bs.Actor:
        ...
        mathnode = bs.newnode(
            "math", owner=spaz.node, attrs={"input1": (0, 1.4, 0), "operation": "add"}
        )
