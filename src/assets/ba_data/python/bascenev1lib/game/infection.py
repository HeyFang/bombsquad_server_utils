    ...
    def handlemessage(self, msg: Any) -> Any:
        if isinstance(msg, bs.DieMessage):
            if not self.died:
                ...
                self.died = True
                bs.animate_array(self.zone, "size", 1, {0: [2 * self.rad], 0.05: [0]})
                self.zone = None
...
class Infection(bs.TeamGameActivity[Player, Team]):
    ...
    def on_begin(self) -> None:
        ...
        bs.timer(
            self._extra_mines * 1.0, bs.WeakCall(self._max_mine_update), repeat=True
        )
    ...
    def spawn_player(self, player: PlayerType) -> bs.Actor:
        ...
        spaz.connect_controls_to_player(
            enable_punch=False, enable_bomb=self._enable_bombs, enable_pickup=False
        )
    ...
    def spawn_player_spaz(
        ...
        spaz = PlayerSpaz(
            color=color, highlight=highlight, character=player.character, player=player
        )
