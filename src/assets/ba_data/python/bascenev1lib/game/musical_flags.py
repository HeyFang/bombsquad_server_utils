...
class MFGame(bs.TeamGameActivity[Player, Team]):
            ...
            for player in self.spawned:
                if not player.done:
                    try:
                        ...
                        # Faster elimination: quick freeze then shatter.
                        bs.timer(
                            0.2, bs.Call(player.actor.handlemessage, bs.FreezeMessage())
                        )
                        bs.timer(
                            0.9,
                            bs.Call(
                        ...
                        pass
            # Quicker round reset.
            bs.timer(1.1, self.killRound)
            bs.timer(1.2, self.makeRound)
            self.round_time_textnode.opacity = 0
    ...
    def makeRound(self):
        ...
        for i in range(c - 1):
            ...
            flag = Flag(position=(x + 0.5, 5, z - 4), color=colors[i]).autoretain()
    ...
    def spawn_player(self, player: Player, pos: tuple = (0, 0, 0)) -> bs.Actor:
        ...
        spaz.connect_controls_to_player(
            enable_punch=self.is_punch, enable_bomb=False, enable_run=self.is_run
        )
    ...
    def check_respawn(self, player):
        if player and player.exists() and not player.done and player.survived:
            # Faster respawn to keep flow snappy between rounds.
            self.respawn_player(player, 1.0)
                ...
                for player in self.spawned:
                    if not player.done:
                        try:
                            ...
                            player.actor.handlemessage(bs.StandMessage((0, 3, -2)))
                            # Faster elimination on round end.
                            bs.timer(
                                0.2,
                                bs.Call(player.actor.handlemessage, bs.FreezeMessage()),
                            )
                            bs.timer(
                                0.9,
                                bs.Call(
    ...
    def checkEnd(self):
        ...
        if i <= 1:
            ...
            # End game sooner once a winner is determined.
